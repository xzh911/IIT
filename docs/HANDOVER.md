...
## 30. r8 候选批次（2026-08-19 23:00）— 用户 r7 真机确认后新反馈（7 项，仅分析+交接，未动工）

**背景**：用户确认「这一版很好 基础矛盾解决了一个」= **r7 的 .ios 注入已修好状态栏重叠**（§29 方案正确）。随后提出 7 项新问题。本轮只做定位分析，未改代码。用户原话全文 = `docs/FEEDBACK-2026-08-19.md` §9。

### 逐项分析（锚点已实锤，下轮勿重探）

| # | 问题 | 已定位 | 方案 / 工作量 |
|---|------|--------|--------------|
| 1 | 纳税人识别号显示/隐藏无效（隐藏态应=头尾各 1 数字 + 中间 16 个 `*`，即 `3****************6`） | 「纳税人识别号」文案在 deob：`commons/YAH1.js:29,103`（完税证明详情，`纳税人识别号：`+div 渲染）、`commons/v17G.js:109,210`、`chunks/119/BUsO.js`、`chunks/14/NR1c.js:86`（x-input 输入场景）。toggle 眼睛按钮 handler 与脱敏函数**未定位** | bundle-investigator 查眼睛按钮 handler + mask 逻辑（官方前端做还是信后端脱敏字段）；确认页面归属（完税证明详情 or 纳税记录开具 or 个人信息）后 fixture/overlay 修。中 |
| 2 | 个人信息页所有项不可编辑（证件号码应灰只读 `3****************6`；所在地区点进去没得选；详细住址无法输入；学历/民族点不开） | 编辑路由与消费接口**未定位**。推断：地区树/学历字典(mzDm/xlDm)/提交接口 3-4 个 miss → picker 空/点击无反应；证件号码现为明文可输入态（fixture `sfzjhm` 直出或未登录分支） | 派 bundle-investigator：定位个人信息编辑页 chunk + 接口清单 → 补 fixture（区域树、学历、民族、保存）。中 |
| 3 | /dev 任意处点 5 下误触 | **根因已实锤**：`web/overlays/dev-entry.js:473-498` document **capture 阶段全局 click** + `textContent.indexOf('用户注册协议'/'自然人办税服务平台')` 祖先链匹配计数——这两行文字挂在全局 footer，几乎处处命中 | 改触发：仅「首页 tab 栏『我的』item」5 连击（条件:`route==/zdj-home` 或 `el.closest(tabbar 类)` 文本=='我的'；tabbar 类名待 grep）。低，0.5h |
| 4 | 纳税记录开具页：随机验证码（非滑块）+ 证件号码显示「请输入」可编辑 | `GET web/common/captcha/base64Image` 为 miss（旧约束「勿 mock 验证码」→ **用户已反转该决策**，见下）；滑块组件复刻内**不存在**（grep 滑块/拖拽/captcha 无 overlay 实现，完税证明链滑动交互也未组件化）；「请输入」= 未登录/未实名分支或 fixture 缺字段 | a) 定位该页 chunk + captcha 组件来源（bundle 内是否内置滑块拼图组件）；b) 实现官方样式滑块：`请按住滑块，拖动到最右侧`→滑动中左侧绿色→完成绿勾「验证通过」（用户原话 §9，参考 reference 截图）；c) 证件号码改灰只读 `3****************6`。中~高 |
| 5 | 去掉「2 个待办 + 2 个消息」 | 待办数据 = `global-shared.json:197` `fpTtxxList`（现 3 条：汇算提醒/专项附加扣除/退税进度）；消息 = `message-list.json`（3 条）。用户说 2+2 与现 3+3 有出入 → **指哪两处待下轮截图确认**（可能：首页角标数字/待办页列表/消息页列表） | 确认后：对应 fixture 置空 + 未读计数接口清零。低 |
| 6 | 手势密码难度评估：「看看原版在哪调用，照猫画虎」 | **定论：bundle 内无「手势」字样**。仅有：`安全中心`（deobfuscated.js:37676 / commons/myBO.js:1212）+ 指纹/扫脸生物验证（deobfuscated.js:27029-27451 多处「请验证指纹/扫脸」+ 锁屏逻辑，登录/敏感操作时调用）。**官方原版没有手势密码功能** | 若做 = 纯自定义层新功能（9 宫格设置页 + 启动拦截 + localStorage），约 0.5-1 天含验证，且偏离官方保真。**建议暂缓**（用户已给台阶「工作量大的话就算了」）；坚持做时入口仿「安全中心」样式，注入点 = config-overrides.js / 覆盖层路由 |
| 7 | /dev 面板超级难用 → 仿照另一个仿冒应用的样子和逻辑 | 参考源 = `reference/alternate-web/site-8082/`（试用站宫格首页 UI，8080/8081 同目录） | 面板重做：宫格化首页（大图标+文字 2 列：fixture 模式切换 / 模板速填 / 路由导航 / 重置 / 请求日志 / 高级表单折叠）；**契约 `{match,method,data}` 不变**（兼容既有 localStorage 覆盖层）；触发一并改（问题 3）。中，~半天 |

**决策变更**：`constraint.captcha_miss_not_mock`（captcha/base64Image 保持 miss）**被用户需求反转**——用户明确要滑块验证（§9 原话）。旧约束作废，滑块 mock 策略待组件定位后定（勿直接 mock 一张验证码图骗过页面，要做真实可交互滑块）。已同步更记忆。
