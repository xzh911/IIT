...
> ⚠️ **状态标注（2026-08-19）**：本文档为 08-18 进度总览，**部分内容已过时**（阶段 3 iOS 壳已从"未开始"变为"已出 IPA r5"；dev 面板已升级 V2；20 菜单入口/三小项已完成）。**现行状态 = `docs/STATE.md` + `docs/HANDOVER.md`（§24-§27）**；技术原理（§4 逆向结论、§9 踩坑清单）仍有效可复用。
